...
import { FaFilePdf, FaGithub, FaQuoteLeft, FaTimes } from 'react-icons/fa';
...
const PublicationDetailsModal = ({ publication, onClose }) => {
  ...
  if (!publication || !modalRoot) return null;

  const buttons = [
    publication.showCodeButton && publication.codeUrl && {
      label: 'Code',
      href: publication.codeUrl,
      icon: FaGithub,
    },
    publication.showPdfButton && publication.pdfUrl && {
      label: 'Paper',
      href: publication.pdfUrl,
      icon: FaFilePdf,
    },
    publication.showDoiButton && publication.doi && {
      label: 'DOI',
        ...
        >
          ...
          >
            <div ref={scrollContentRef} className="space-y-6 p-8">
              ...
              <h3 className="text-2xl font-semibold tracking-tight">{publication.title}</h3>
              <p className="text-sm text-white/70">{publication.venue}{publication.year ? ` · ${publication.year}` : ''}</p>
              {publication.authors && (
                <p className="text-sm text-white/60">
                  <span className="font-medium text-white/80">Authors:</span> {publication.authors}
                </p>
              )}
            </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-white/12 bg-black/40">
              {publication.thumbnailUrl ? (
                ...
                <div className="h-48 w-full bg-gradient-to-br from-purple-500/20 via-transparent to-blue-500/20" />
              )}
            </div>
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm leading-relaxed text-white/80 whitespace-pre-line">
                {publication.abstract || 'Abstract unavailable for this publication.'}
              </p>

              {publication.showCitations && publication.citationCount > 0 && (
                <div className="flex items-center gap-2 text-sm text-white/60">
                  <FaQuoteLeft className="h-3 w-3" />
                  {publication.citationCount} citation(s)
                </div>
              )}
            </div>
